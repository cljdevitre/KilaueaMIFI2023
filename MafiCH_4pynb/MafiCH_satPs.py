#This is a modification of the the script for the MafiCH model of Allison et al. 
#(Contributions to #Mineralogy and Petrology), aimed to be run as module in 
# Jupyter Lab notebooks. Functions are callable. 

# FYI, CHANGES:
# 1) You do not need a specific file structure if you load this as a module in 
# Jupyter Lab Notebook. 
# 2) A function has been added to convert excel columns to 
# the required format needed by MafiCH. 
# 3) The part of the script that runs the model has been converted to a callable
# function. 

# 4) No part of this module needs to be edited unless Allison et al., modify the 
# input file structure (in which case, modify the column names in default_cols)
...
def create_mafich_inputfile(sample_file,sample_cols):
    ## This creates the default input file for MafiCH -- DO NOT EDIT

    mafich_input = pd.DataFrame(columns=default_cols)

    # This matches the columns from sample to default input -- DO NOT EDIT
    col_mapping = {
        default_cols[0]: sample_cols[0],
        default_cols[1]: sample_cols[1],
        default_cols[2]: sample_cols[2],
        default_cols[3]: sample_cols[3],
        default_cols[4]: sample_cols[4],
        default_cols[5]: sample_cols[5],
        default_cols[6]: sample_cols[6],
        default_cols[7]: sample_cols[7],
        default_cols[8]: sample_cols[8],
        default_cols[9]: sample_cols[9],
        default_cols[10]: sample_cols[10],
        default_cols[11]: sample_cols[11],
        default_cols[12]: sample_cols[12]
    }

    # Fill mafich_input with the values from sample_file based on column mapping

    for col in mafich_input.columns:
        mafich_input[col] = sample_file[col_mapping[col]].tolist()
    # Drop NaN rows
    mafich_input.dropna(inplace=True)
    mafich_input.reset_index(drop=True, inplace=True)

    display(mafich_input)
    
    mafich_input.to_csv('MafiCH_satpress_input.csv',index=False)
...
################################################################################
#This function is used to run the model on samples. 
################################################################################


def run_mafich(satpressinput,satpressoutput):
    ...
    new_cols=default_cols+['Pressure (bar)', 'XfH2O']
    satpress_outputdf = pd.DataFrame(satpressoutputlist, columns=new_cols)

    # Save results as csv file and also returns the pandas dataframe to the notebook.
    satpress_outputdf.to_csv(satpressoutput, index=False)
    return satpress_outputdf
